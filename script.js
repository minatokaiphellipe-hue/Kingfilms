...
const getIcon=(c)=>{
  let l=c.toLowerCase();
  if(l.includes('oscar'))return '🏆'; if(l.includes('filme'))return '🎬';if(l.includes('série'))return '📺';
  if(l.includes('anime'))return '⛩️';if(l.includes('animaç'))return '🦄';if(l.includes('desenho'))return '🖍️';
  if(l.includes('ação'))return '💥';if(l.includes('comédia'))return '😂';if(l.includes('terror')||l.includes('horror'))return '👻';
...
function criarCartaoHTML(item, isHorizontal=false){
  ...
  return `<div class="content-card" onclick="abrirDetalhes('${item.id}')"><div style="position:relative;width:100%;"><img src="${imgCapa}" alt="${item.titulo}" loading="lazy" style="display:block;width:100%;"></div><div class="card-info"><h3>${item.titulo.replace(" 🌀","")}</h3><div class="btn-assista">▶ Assista Já</div></div></div>`;
}
function gerarTop10(){
...
function renderizarHome(){
  ...
  cats.forEach(c=>{ if(!c.toLowerCase().includes('oscar')) hSidebar+=`<div class="sidebar-item" onclick="fecharMenu(); verCategoriaCompleta('${c}')">${getIcon(c)} ${c}</div>`; });
  ...
  if(itensOscar.length > 0){
      hFeed+=`<div class="oscar-section" id="oscar-2026"><h2 class="oscar-title"><span class="oscar-icon">🏆</span> Vencedores do Oscar</h2><div class="content-row">`;
      itensOscar.forEach(item=>{hFeed+=criarCartaoHTML(item, false);});
      hFeed+=`</div></div>`;
  }
  ...
  rec.forEach(i=>{hFeed+=criarCartaoHTML(i, false);}); hFeed+=`</div></div>`;
  
  cats.forEach(c=>{
    if(c.toLowerCase().includes('oscar')) return;
    ...
    iCar.forEach(i=>{hFeed+=criarCartaoHTML(i, false);}); hFeed+=`</div>`;
...
function verCategoriaCompleta(c){
  ...
  iC.forEach(i=>{h+=criarCartaoHTML(i, false);}); h+=`</div>`; f.innerHTML=h; window.scrollTo({top:0,behavior:'smooth'});
}
function pesquisarConteudo(){
  ...
  r.forEach(i=>{h+=criarCartaoHTML(i, false);}); h+=`</div>`; f.innerHTML=h;
}
...
function abrirDetalhes(id){
  ...
  if(rel.length>0){
    ...
    rel.forEach(r=>{hR+=criarCartaoHTML(r, false);}); hR+=`</div></div>`;
  }
