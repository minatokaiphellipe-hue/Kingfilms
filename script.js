...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const t = Date.now();
        const p = [
            fetch('catalogo_filmes.json?v='+t).catch(()=>null),
            fetch('catalogo_series.json?v='+t).catch(()=>null),
            fetch('catalogo_animes.json?v='+t).catch(()=>null),
            fetch('catalogo_pro.json?v='+t).catch(()=>null)
        ];
        const [rF, rS, rA, rO] = await Promise.all(p);

        let f = (rF && rF.ok) ? await rF.json().catch(()=>[]) : [];
        let s = (rS && rS.ok) ? await rS.json().catch(()=>[]) : [];
        let a = (rA && rA.ok) ? await rA.json().catch(()=>[]) : [];
        let o = (rO && rO.ok) ? await rO.json().catch(()=>[]) : [];

        todosDados = [...f, ...s, ...a];
        
        // SISTEMA DE SALVAÇÃO: Se os novos estiverem vazios, usa o antigo!
        if (todosDados.length === 0 && o.length > 0) {
            todosDados = o;
        }
        ...
        if (todosDados.length > 0) {
            ...
            document.getElementById('feedPrincipal').innerHTML = `<p style="color:#aaa; text-align:center; padding:50px;">Catálogo vazio. Publique novos conteúdos no Painel!</p>`;
        }
    } catch (e) { 
        document.getElementById('feedPrincipal').innerHTML = `<p style="color:red; text-align:center; padding:50px;">Erro: ${e.message}</p>`;
    }
});
...
// --- NOVA LÓGICA DAS ABAS DE TEMPORADA ---
function mudarTemporada(idItem, tempDesejada) {
    ...
    // Renderiza os episódios da nova temporada escolhida
...
function abrirDetalhes(idItem) {
    ...
    // Garante que as temporadas são ordenadas numericamente (1, 2, 3... 10)
    const temps = Object.keys(item.temporadas).sort((a,b) => parseInt(a) - parseInt(b));
    
    // Constrói os botões horizontais em vez do <select>
    let selectTemporada = "";
    if (temps.length > 1) {
...
function renderEps(idItem, temporadaDesejada) {
    ...
    // Se for filme (não tem temporadaDesejada enviada pelo clique), pega do input oculto
    ...
    document.getElementById('listaEps').innerHTML = item.temporadas[temporadaDesejada].map(e => `
        <div style="display:flex; align-items:center; gap:15px; background:rgba(20,20,20,0.6); padding:12px; margin-bottom:12px; border-radius:10px; cursor:pointer; border: 1px solid rgba(255,255,255,0.05); transition: 0.3s; backdrop-filter: blur(5px);" 
            ...
            <div style="flex:1; overflow:hidden;">
                ...
                <div style="display:inline-block; background: linear-gradient(45deg, #ff0a16, #8b0000); padding: 4px 10px; border-radius: 4px; font-size:12px; font-weight:bold; color:#fff; box-shadow: 0 2px 5px rgba(229, 9, 20, 0.4);">▶ Assista no Rave</div>
            </div>
        </div>`).join('');
}
