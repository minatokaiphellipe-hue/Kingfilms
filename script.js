...
window.logarComGoogle = function() {
    ...
    .catch((error) => {
        ...
        erro.innerText = "Lembre-se de adicionar o domínio nas configurações do Firebase.";
    });
};
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('feedPrincipal').innerHTML = `
...
// ⚡ OTIMIZAÇÃO
todosDados = data.reverse();

// 🔥 limitar por categoria (não quebra o site)
let limitePorCategoria = 50;
...
todosDados.forEach(item => {
    ...
    if (mapa[cat].length < limitePorCategoria) {
        mapa[cat].push(item);
    }
});

// junta tudo de novo
todosDados = Object.values(mapa).flat(); // pode ajustar (200, 500 etc)
...
window.playVid = function(id, link) {
    ...
    // 2. Leva o utilizador para a URL exata do filme (aqui foi corrigido)
...
function criarCartaoHTML(item,isHorizontal=false){
  ...
  let imgCapa = isHorizontal 
    ? (item.logo_horizontal || item.logo) 
    : (item.logo || item.logo_horizontal || item.capa || item.thumb);

  // 🔥 fallback seguro
  if (!imgCapa) {
    imgCapa = "https://via.placeholder.com/300x450?text=Sem+Capa";
  }

  // 🔥 corrige caminho relativo
  if (imgCapa && !imgCapa.startsWith("http")) {
    imgCapa = "./" + imgCapa;
  }

  let notaBadge = (item.avaliacao && item.avaliacao !== "0.0" && item.avaliacao !== "N/A") 
  ? `<div style="position:absolute; top:10px; right:10px; background:rgba(0,0,0,0.5); color:#fff; padding:4px 8px; border-radius:8px; font-size:12px; font-weight:700;">★ ${item.avaliacao}</div>` 
  : "";

  let classCor = "#0f0"; 
  let tC = item.classificacao || "L";

  if(tC==="Livre"||tC==="L"){classCor="#10b981";tC="L";}
  else if(tC==="10")classCor="#0ea5e9";
  else if(tC==="12")classCor="#f59e0b";
  else if(tC==="14")classCor="#f97316";
  else if(tC==="16"||tC==="18")classCor="#ef4444";

  let classBadge = `<div style="position:absolute; top:10px; left:10px; background:${classCor}; color:#fff; padding:3px 6px; border-radius:6px; font-size:11px; font-weight:800;">${tC}</div>`;
  ...
  <div class="content-card" onclick="abrirDetalhes('${item.id}')">
    <div style="position:relative;width:100%;">
      <img src="${imgCapa}" 
           ...
           onerror="this.src='https://via.placeholder.com/300x450?text=Erro'">
      ${notaBadge}
      ${classBadge}
    </div>
    <div class="card-info">
...
listaAtual = todosDados.filter(i => {
    return i.titulo && i.titulo.toLowerCase().includes(termo);
});itensExibidos=ITENS_POR_PAGINA;
...
proximos.forEach(i => {
    ...
    div.innerHTML = criarCartaoHTML(i,false);
    fragment.appendChild(div.firstChild);
});

grid.appendChild(fragment);grid.insertAdjacentHTML('beforeend',h);itensExibidos+=ITENS_POR_PAGINA;
