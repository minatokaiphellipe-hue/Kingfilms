...
/* FAVORITOS COM HEARTBEAT (Efeito Pulsar) */
window.addLista = function(id) {
    ...
    if(document.getElementById('btnListaFlutuante')){
        ...
        if(inList) {
            ...
            setTimeout(() => btn.classList.remove('heartBeat'), 500);
        }
    } else {
        renderizarHome(false);
    }
...
/* CARTÕES ORIGINAIS, APENAS COM LAZY-IMG CLASSE ADICIONADA */
function criarCartaoHTML(item,isHorizontal=false){
  ...
  return `<div class="content-card" onclick="abrirDetalhes('${item.id}')"><div style="position:relative;width:100%;"><img src="${imgCapa}" alt="${item.titulo}" loading="lazy" class="lazy-img" onload="this.classList.add('loaded')" style="display:block;width:100%;">${notaBadge}${classBadge}</div><div class="card-info"><h3>${item.titulo.replace(" 🌀","")}</h3></div></div>`;
}
...
function gerarTop10(){
  ...
  t10.forEach((i,x)=>{h+=`<div class="top10-card" onclick="abrirDetalhes('${i.id}')"><div class="top10-number">${x+1}</div><div class="top10-img-box"><img src="${i.logo}" alt="${i.titulo}" loading="lazy" class="lazy-img" onload="this.classList.add('loaded')"></div></div>`;});return h+`</div></div>`;
}
...
function renderizarHome(salvarUrl=true){
  ...
  let hFeed="<div class='fade-in'>"; // APLICAÇÃO DA ANIMAÇÃO FADE-IN
  ...
  hFeed+="</div>"; // FECHA O FADE-IN WRAPPER
  f.innerHTML=hFeed;
...
function verCategoriaCompleta(c,salvar=true){
  ...
  f.innerHTML=h + `</div>`;window.scrollTo({top:0,behavior:'smooth'});
}
...
function pesquisarConteudo(salvar=true, ignorarPilha=false){
  ...
  if(listaAtual.length===0){f.innerHTML=`<div class="fade-in" style="padding:15px 5%;"><button onclick="voltarPagina()" style="background:#e50914;color:white;border:none;padding:10px 20px;border-radius:8px;cursor:pointer;margin-bottom:20px;font-weight:bold;box-shadow:0 4px 10px rgba(229,9,20,0.3);">← Voltar</button></div><div class="fade-in" style="text-align:center;margin-top:60px;color:#888;"><span style="font-size:45px;">😕</span><br><br><p>Nenhum resultado para "<b>${t}</b>"</p></div>`;return;}
