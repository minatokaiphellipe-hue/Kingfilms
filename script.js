...
function renderizarHome(salvarUrl=true){
  ...
  /* --- TOP 10 --- */
  hFeed+=gerarTop10();
  /* --- 2. SHOWCASE NEON FLUTUANTE (COM ÍCONE PLAY REDONDO NO CENTRO) --- */
  if(todosDados.length > 5){
    let destShowcase = todosDados.slice(5, 9); // Pega os 4 seguintes
    if(destShowcase.length < 4) destShowcase = todosDados.slice(0, 4); // Backup
    ...
    destShowcase.forEach((d,i)=>{
      ...
      // Adiciona as classes active, prev e next no primeiro carregamento
      let classCSS = "";
      if(i === 0) classCSS = "active";
      else if(i === 1) classCSS = "next";
      else if(i === destShowcase.length - 1) classCSS = "prev";

      // Toda a div recebe a ação de clicar: se tiver no canto ele puxa pro meio, se tiver no meio ele abre o filme!
      hFeed+=`
      <div class="showcase-slide ${classCSS}" id="s-slide-${i}" onclick="if(!this.classList.contains('active')) mudarShowcase(${i}); else abrirDetalhes('${d.id}');">
          <div class="showcase-img-box">
              <img src="${d.logo}" alt="${d.titulo}"> <!-- Usa o pôster vertical -->
              <div class="showcase-overlay"></div>
              <div class="showcase-play-icon"></div> <!-- O BOTÃO DE PLAY FICA AQUI DENTRO AGORA -->
          </div>
...
/* Lógica Independente para o HERO GIGANTE */
...
/* LÓGICA: SHOWCASE COVER FLOW (NEON) */
