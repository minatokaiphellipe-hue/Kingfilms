...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const res = await fetch('catalogo_pro.json?v=' + Date.now()); 
        todosDados = await res.json();
        todosDados.reverse(); 
        renderizarHome();
    } catch (e) { 
        document.getElementById('feedPrincipal').innerHTML = `<p style="color:red; text-align:center; padding:50px;">Erro ao carregar catálogo.</p>`;
    }
});
