...
// ==========================================================
// 👑 FUNÇÃO DE BRANDING INTELIGENTE (Apenas na URL /v/)
// ==========================================================
window.formatarTituloKingfilms = function(titulo) {
    if (!titulo) return "";
    
    // 1. Limpa sempre o redemoinho de qualquer título
    let tituloLimpo = titulo.replace(/\s*🌀/g, "").trim();
    
    // 2. Verifica se a URL atual contém a pasta '/v/' (Página do Player)
    if (window.location.href.includes('/v/')) {
        // Injeta a marca apenas se ela ainda não existir
        if (tituloLimpo.includes('𝐊𝐢𝐧𝐠𝐟𝐢𝐥𝐦𝐬 👑')) {
            return tituloLimpo;
        }
        return `${tituloLimpo} - 𝐊𝐢𝐧𝐠𝐟𝐢𝐥𝐦𝐬 👑`;
    }
    
    // 3. Se for o feed principal, remove a coroa caso venha do JSON e devolve o nome limpo
    return tituloLimpo.replace(" - 𝐊𝐢𝐧𝐠𝐟𝐢𝐥𝐦𝐬 👑", "").trim();
};
...
function criarCartaoHTML(item,isHorizontal=false){
  ...
  return `<div class="content-card" onclick="abrirDetalhes('${item.id}')"><div style="position:relative;width:100%;"><img src="${imgCapa}" alt="${item.titulo}" loading="lazy" style="display:block;width:100%;">${notaBadge}${classBadge}</div><div class="card-info"><h3>${formatarTituloKingfilms(item.titulo)}</h3></div></div>`;
}
...
function renderizarHome(salvarUrl=true){
  ...
  let hFeed="";if(todosDados.length>0){
    let dest=todosDados.slice(0,5);hFeed+=`<div class="hero-slider" id="heroSlider">`;
    dest.forEach((d,i)=>{let cC="#0f0",tC=d.classificacao||"L";if(tC==="Livre"||tC==="L"){cC="#0f0";tC="Livre";}else if(tC==="10")cC="#00a5ff";else if(tC==="12")cC="#ffcc00";else if(tC==="14")cC="#ff6600";else if(tC==="16"||tC==="18"||tC.includes("TV-MA")||tC.includes("R"))cC="#e50914";let nH=d.avaliacao&&d.avaliacao!=="0.0"&&d.avaliacao!=="N/A"?`<span style="color:#46d369;">★ ${d.avaliacao}</span>`:"";hFeed+=`<div class="hero-slide ${i===0?'active':''}" id="slide-${i}" style="background-image:url('${d.logo_horizontal||d.logo}');"><div class="hero-overlay"></div><div class="hero-content"><h1 class="hero-title">${formatarTituloKingfilms(d.titulo)}</h1><div class="hero-meta">${nH}<span style="background:${cC};color:#000;padding:3px 8px;border-radius:4px;">${tC}</span><span style="color:#ccc;">${d.tipo==='filme'?'Filme':'Série'}</span></div><p class="hero-synopsis">${d.sinopse||'Assista agora no KINGFILMS.'}</p><div class="hero-buttons"><button class="btn-hero-play" onclick="abrirDetalhes('${d.id}')">▶ Assistir</button><button class="btn-hero-info" onclick="abrirDetalhes('${d.id}')">ℹ Mais Info</button></div></div></div>`;});
    hFeed+=`<div class="slider-dots">`;dest.forEach((_,i)=>{hFeed+=`<div class="dot ${i===0?'active':''}" id="dot-${i}" onclick="mudarSlide(${i})"></div>`;});hFeed+=`</div></div>`;setTimeout(()=>{iniciarSlider(dest.length);configurarSwipe();},100);
  }
