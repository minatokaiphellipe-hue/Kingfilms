...
function criarCartaoHTML(item,isHorizontal=false){
  ...
  return `<div class="content-card" onclick="abrirDetalhes('${item.id}')"><div style="position:relative;width:100%;"><img src="${imgCapa}" alt="${item.titulo}" loading="lazy" style="display:block;width:100%;">${notaBadge}${classBadge}</div><div class="card-info"><h3>${item.titulo.replace(" 🌀","")}</h3><div class="btn-assista">▶ Assista Já</div></div></div>`;
}
function gerarTop10(){
